-module(ezx_memory_128_banks_tuples).

%% @doc 128K memory backend: record-based state, a 4-slot routing tuple, and
%% a banks tuple of 16KB banks — each bank a tuple of 512-byte *pages stored
%% as byte tuples* (512-element tuples) instead of binaries.
%%
%% This is the same layout as ezx_memory_128_banks but with per-page storage
%% as a 512-element tuple of bytes, so a byte read is a single element/2 on
%% the page tuple and a byte write a single setelement/2 — instead of a
%% binary pattern match / binary rebuild. It exists to A/B the two storage
%% representations (binary page vs byte-tuple page) for the same geometry.
%%
%% == Memory map (CPU view) ==
...
%%
%% == Structure ==
%% state() is a record. The banks live in a plain 10-element tuple (2 ROM
%% banks + 8 RAM banks); each bank is a 32-element tuple of 512-byte pages;
%% each page is a 512-element tuple of bytes. Variants of per-bank storage
%% could be plugged in later without touching the routing logic.
%%
...
write_byte(#mem128{routing = Routing, banks = Banks} = State, Addr, Byte) ->
    ...
    PageIdx = ((Addr bsr ?PAGE_BITS) band (?BANK_PAGES - 1)) + 1,
    Off = Addr band (?PAGE_SIZE - 1),
    NewPage = setelement(Off + 1, element(PageIdx, Bank), Byte),
...
%% The buffer size is fixed, so no size argument is needed. Only the byte
%% tuples that intersect the screen region are flattened to a single binary
%% (13 full 512-byte pages + the first 256 bytes of the 14th), instead of
%% the whole 16KB bank.
...
read_bank_block(#mem128{banks = Banks}, Bank) ->
    iolist_to_binary([page_to_binary(P) ||
                        P <- tuple_to_list(element(Bank + ?RAM_BASE_IDX
                                                      + ?BANK_INDEX_OFFSET,
                                                  Banks))]).
