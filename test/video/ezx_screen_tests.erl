...
output_size_test() ->
    ...
    RGB = ezx_screen:render_frame(VB, false, [], 1),
...
flash_test() ->
    ...
    RGB = ezx_screen:render_frame(VB, true, [], 1),
    ?assertEqual({0, 0, 0}, read_pixel(RGB, 48, 48)).
...
border_no_changes_test() ->
    VB = create_video_buffer(16#00, 16#00),
    RGB = ezx_screen:render_frame(VB, false, [], 5),
    ?assertEqual({0, 215, 215}, read_pixel(RGB, 0, 0)).

uniform_border_test() ->
    VB = create_video_buffer(16#00, 16#00),
    RGB = ezx_screen:render_frame(VB, false, [], 4),
    C = color(4),
...
border_change_test() ->
    VB = create_video_buffer(16#00, 16#00),
    RGB = ezx_screen:render_frame(VB, false, [{0, 2}], 1),
    ?assertEqual({215, 0, 0}, read_pixel(RGB, 0, 0)).

border_stripes_test() ->
    VB = create_video_buffer(16#00, 16#00),
    RGB = ezx_screen:render_frame(VB, false, stripes_changes(), 7),
